...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initClientDownloads() {
        ...
        clientCards.forEach(card => {
            card.addEventListener('click', function() {
                const clientType = this.classList[1]; // windows, ios, android, macos
                ...
                setTimeout(() => {
                    this.style.transform = '';
                    showNotification(`🚀 正在准备 ${clientName} 下载...`, 'info');
                    
                    // 这里可以添加实际的下载逻辑
                    handleClientDownload(clientType);
                }, 200);
            });
        });
    }
    ...
    // 处理客户端下载
    function handleClientDownload(clientType) {
        const downloadUrls = {
        ...
        const url = downloadUrls[clientType];
        if (url && url !== '#') {
            window.open(url, '_blank');
        } else {
            showNotification('📱 下载链接准备中，请稍后再试', 'warning');
        }
    }
